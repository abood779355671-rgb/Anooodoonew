...
class Config:
    ...
    def __init__(self):
        ...
        # Optional external API used as an alternative to yt-dlp when YouTube
        # blocks datacenter IPs.  Both audio and video use the same API_KEY.
        # - API_URL      : base URL for audio downloads  (e.g. https://api.example.com)
        # - VIDEO_API_URL: base URL for video downloads  (may be the same host)
        # - API_KEY      : authentication key for the above API
        # Leave all three empty to disable (bot falls back to yt-dlp only).

...
class YouTube:
    ...
    def _use_video_api(self) -> bool:
        return bool(config.VIDEO_API_URL and config.API_KEY)
    ...
    async def _download_file_from_url(self, url: str, out_path: str) -> Optional[str]:
        """Stream a remote file to disk and return the path on success."""
        if not url:
            return None
        try:
    ...
    async def _api_download_audio(self, link: str) -> Optional[str]:
        """
        Download audio via the external API (API_URL).
        Polls until the server reports "done", then streams the file to disk.
        ...
        if not vid:
            return None
        poll_url = f"{config.API_URL}/song/{vid}?api={config.API_KEY}"
        try:
            session = await self._get_api_session()
            while True:
                ...
                out_path = f"downloads/{vid}.{fmt}"
                result = await self._download_file_from_url(dl_url, out_path)
                if result:
                    logger.info(f"✅ Audio for {vid} downloaded via external API")
                return result
        except Exception:
            return None
    ...
    async def _api_download_video(self, link: str) -> Optional[str]:
        """
        Download video via the external API (VIDEO_API_URL).
        Polls until the server reports "done", then streams the file to disk.
        Returns the local file path on success, None otherwise.
        """
        ...
        if not vid:
            return None
        poll_url = f"{config.VIDEO_API_URL}/video/{vid}?api={config.API_KEY}"
        try:
            session = await self._get_api_session()
            while True:
                async with session.get(poll_url) as r:
                ...
                result = await self._download_file_from_url(dl_url, out_path)
                if result:
                    logger.info(f"✅ Video for {vid} downloaded via external API")
                return result
        except Exception:
            return None

    async def _race_ytdlp_and_api(
        self,
        yt_task: asyncio.Task,
        api_task: asyncio.Task,
        video_id: str,
        source_label: str,
    ) -> Optional[str]:
        """
        Run yt-dlp and external API in parallel.
        Returns the first valid file path; cancels the loser.
        """
        ...
        for task in done:
            ...
            if result and os.path.exists(result):
                src = "yt-dlp" if task is yt_task else "external API"
                logger.info(f"✅ {source_label} '{video_id}' — served by {src}")
                for p in pending:
                    p.cancel()
                    with contextlib.suppress(asyncio.CancelledError):
                        await p
                return result
        # First finisher had no result — wait for the pending one
        for task in pending:
            try:
                result = await task
                if result and os.path.exists(result):
    ...
    def _build_ytdlp_search_opts(self, cookie: Optional[str] = None) -> dict:
        ...
        opts = {
            ...
            # ios + mweb are listed as fallbacks: some videos block the android
            # client (causing "Requested format is not available") while still
            # serving ios or mweb — listing all three ensures yt-dlp tries them
    ...
    async def search(self, query: str, m_id: int) -> Track | None:
        ...
        try:
            if self.valid(query):
                ...
                track = None
                try:
                    # ytlookup's videosearch is an async function (coroutine),
                    # NOT a class with a .next() method like youtube-search-python.
                    # Calling videosearch(...) without await returns a coroutine
                    # object — that's why ".next()" raised AttributeError.
                    # Await it directly to get the result dict.
                    results = await asyncio.wait_for(videosearch(query, limit=1), timeout=8)
    ...
    async def download(self, video_id: str, is_live: bool = False, video: bool = False) -> Optional[str]:
        ...
        if is_live:
            ...
            ydl_opts = {
                ...
                # Use android + ios + mweb clients to bypass YouTube bot detection.
                # ios is essential here: live streams often only expose HLS manifests
                # via the ios client, which android may report as unavailable.
                "extractor_args": {"youtube": {"player_client": ["android", "ios", "mweb"]}},
            }
            if getattr(config, "YTDLP_PROXY", ""):
        ...
        async with self._download_semaphore:
            ...
            base_opts = {
                ...
                "sleep_interval_requests": 1,
                # Use android + ios + mweb clients to bypass YouTube bot detection.
                # "Requested format is not available" is thrown when the android client
                # cannot serve a particular video's formats — adding ios and mweb as
                # fallbacks lets yt-dlp retry with a different client automatically.
                "extractor_args": {"youtube": {"player_client": ["android", "ios", "mweb"]}},
            }

            if getattr(config, "YTDLP_PROXY", ""):
                base_opts["proxy"] = config.YTDLP_PROXY
            ...
            if video:
                ...
                # Audio mode: prefer m4a, then webm/opus (ios/mweb clients serve these),
                # then any best audio, then best overall.
                # The narrow "bestaudio[ext=m4a]/bestaudio/best" chain caused
                # "Requested format is not available" when android couldn't serve m4a
                # and no broader fallback was in the format string.
                ydl_opts = {
                    **base_opts,
            ...
            # ── Attempt 1: race yt-dlp (with cookies) against external API ──────
            url_for_api = self.base + video_id
            if video and self._use_video_api():
                yt_task = asyncio.create_task(
                    asyncio.to_thread(_download, ydl_opts_cookie)
                )
                api_task = asyncio.create_task(
                    self._api_download_video(url_for_api)
                )
                result = await self._race_ytdlp_and_api(yt_task, api_task, video_id, "video")
            elif not video and self._use_audio_api():
                yt_task = asyncio.create_task(
                    asyncio.to_thread(_download, ydl_opts_cookie)
                )
                api_task = asyncio.create_task(
                    self._api_download_audio(url_for_api)
                )
                result = await self._race_ytdlp_and_api(yt_task, api_task, video_id, "audio")
            else:
                # No API configured — use yt-dlp with cookies only
                result = await asyncio.to_thread(_download, ydl_opts_cookie)

            if result:
                return result

            # ── Attempt 2: SAME cookies, just widen the format string ──────────
            # "Requested format is not available" almost always means the
            # specific itag we asked for got pulled/changed by YouTube — it is
            # NOT a cookie problem. Removing cookies here (the old behaviour)
            # is what turned this into a guaranteed "Sign in to confirm
            # you're not a bot", since an unauthenticated request from a
            # datacenter IP is exactly what triggers that check.
            logger.warning(
                f"⚠️ First download attempt failed for {video_id}, "
                f"retrying with a wider format (cookies kept)..."
            )
            ydl_opts_wide_format = {**ydl_opts_cookie, "format": "best"}
            result = await asyncio.to_thread(_download, ydl_opts_wide_format)
            if result:
                return result

            # ── Attempt 3: cookies are likely stale/blocked — refresh once ───
            # Force a fresh cookie pull (same mechanism as the scheduled
            # auto-refresh) and try exactly one more time with the new file.
            cookies_url = getattr(config, "COOKIES_URL", None)
            if cookies_url:
                logger.warning(
                    f"⚠️ Second attempt failed for {video_id}, "
                    f"refreshing cookies and retrying once more..."
                )
                try:
                    await self.save_cookies(cookies_url)
                except Exception as e:
                    logger.error(f"❌ Cookie refresh during retry failed: {e}")

                fresh_cookie = self.get_cookies()
                ydl_opts_fresh = {
                    **ydl_opts_cookie,
                    "cookiefile": fresh_cookie,
                    "format": "best",
                }
                result = await asyncio.to_thread(_download, ydl_opts_fresh)
                if result:
                    return result

            logger.error(f"❌ All download attempts exhausted for {video_id}.")
            return None
